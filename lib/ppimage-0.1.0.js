...
(function (_global) {
    ...
    var MODE = {
        ...
        1: {
            method: 1,
            options: {divisor: 1, offset: 0, _matrix: [0, -1, 0, -1, 5, -1, 0, -1, 0, 1, 0]}
        },
        // Mode2:  Emboss               (浮雕)
        2: {
            method: 1,
            options: {divisor: 1, offset: 0, _matrix: [-6, -3, 0, -3, 1, 3, 0, 3, 6, 1, 0]}
        },
        // Mode3:  Contrast-enhancement (增加对比度)
        3: {
            method: 1,
            options: {divisor: 1, offset: 0, _matrix: [0, 0, 0, 0, 2, 0, 0, 0, 0, 1, -255]}
        },
        // Mode4:  Color burn           (颜色加深)
        4: {
            method: 1,
            options: {divisor: 1, offset: 0, _matrix: [0, -2, 0, -2, 20, -2, 0, -2, 0, 10, -40]}
        },
        // Mode5:  Accented Edges       (强化边缘)
        5: {
            method: 1,
            options: {divisor: 1, offset: 0, _matrix: [0, -1, 0, -1, 4, -1, 0, -1, 0, 1, 127]}
        },
        // Mode6:  Blur                 (模糊)
        6: {
            method: 1,
            options: {divisor: 1, offset: 0, _matrix: [0, 1, 0, 1, 1, 1, 0, 1, 0, 5, 0]}
        },
        // Mode7:  Mirror               (镜像)
        7: {
            method: 2,
        ...
        8: {
            ...
            options: {
                divisor: 1,
                offset: 0,
                _param: 'v = _options.offset / _options.divisor;',
                _r: ' r += (100 + v);',
                _g: ' g += (100 + v);',
                _b: ' b += (100 + v);'
            }
        ...
        10: {
            ...
            options: {
                divisor: 1,
                offset: 0,
                _param: 'v = ( r * (.3 + _options.offset) + g * (.59 + _options.offset) + b * (.11 + _options.offset)) / _options.divisor;',
                _r: ' r = v;',
                _g: ' g = v;',
                _b: ' b = v;'
            }
    ...
    var RENDER = {
        ...
        3: function (_input, _options, _opacity) {
            ...
            var _fn = '"use strict";'
                ...
                + (_param ? 'var ' + _param.replace(/\sr\s/igm, 'pixels[i]').replace(/\sg\s/igm, 'pixels[i + 1]').replace(/\sb\s/igm, 'pixels[i + 2]') : '')
                + _options._r.replace(/\sr\s/igm, 'pixels[i]')
                + _options._g.replace(/\sg\s/igm, 'pixels[i + 1]')
                + _options._b.replace(/\sb\s/igm, 'pixels[i + 2]')
            ...
            _image.onload = function () {
                ...
                if (!!_tempCanvas.getContext) {
                    ...
                    if (!!_map.mode) {
                        ...
                        try {
                            var _tmp = {};
                            var _mode = MODE[_map.mode], _param = _mode.options;
                            for (var _option in _param) {
                                if (_param.hasOwnProperty(_option)) {
                                    _tmp[_option] = _param[_option];
                                }
                            }
                            if (_options._options && _options._options.offset) _tmp.offset = _options._options.offset;
                            if (_options._options && _options._options.divisor) _tmp.divisor = _options._options.divisor;

                            var _imageData = _tempCtx.getImageData(0, 0, _map.w, _map.h);
                            var _newImage = RENDER[_mode.method](_imageData, _tmp, _map.opacity);
